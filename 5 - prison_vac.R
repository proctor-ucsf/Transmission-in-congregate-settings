...
ggplot(vac_res %>% filter(trigger == 10)) +
  geom_line(aes(x=vac_delay,y=total_inf,col=as_factor(vac_eff))) +
  geom_line(data = preemp_vac_res, aes(x = vac_delay,y = total_inf,linetype = as_factor(vac_eff))) +
  xlab('Vaccine delay (days)') + ylab('Infections expected per 100 days') +
